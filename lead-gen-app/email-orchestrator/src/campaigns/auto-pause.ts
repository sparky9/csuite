 ...
 */
export async function pauseCampaignProspect(
  prospectId: string,
  campaignId: string,
  reason: string
): Promise<void> {
  try {
    await db.query(
      `UPDATE campaign_prospects
       SET status = 'paused',
           paused_reason = $1
       WHERE prospect_id = $2 AND campaign_id = $3 AND status = 'active'`,
      [reason, prospectId, campaignId]
    );
 ...
 */
export async function resumeCampaignProspect(
  prospectId: string,
  campaignId: string
): Promise<void> {
  try {
    // Calculate next send time based on current sequence
    const campaignProspect = await db.queryOne<{
      current_sequence_order: number;
    }>(
      `SELECT current_sequence_order FROM campaign_prospects
       WHERE prospect_id = $1 AND campaign_id = $2`,
      [prospectId, campaignId]
    );
    ...
    const nextSequence = await db.queryOne<{ day_offset: number }>(
      ...
      [campaignId, campaignProspect.current_sequence_order + 1]
    );
    ...
    await db.query(
      ...
      [nextSendAt, prospectId, campaignId]
    );
 ...
 */
export async function pauseCampaign(campaignId: string, reason: string): Promise<number> {
  try {
 ...
 */
export async function resumeCampaign(campaignId: string): Promise<number> {
  try {
    // Resume campaign status
    await db.query(`UPDATE campaigns SET status = 'active' WHERE id = $1`, [campaignId]);

    // Get all paused prospects
    const pausedProspects = await db.query<{
      id: string;
      prospect_id: string;
      current_sequence_order: number;
    }>(
      `SELECT id, prospect_id, current_sequence_order FROM campaign_prospects
    ...
    for (const cp of pausedProspects.rows) {
      try {
        ...
        const nextSequence = await db.queryOne<{ day_offset: number }>(
          `SELECT day_offset FROM email_sequences
           WHERE campaign_id = $1 AND sequence_order = $2`,
          [campaignId, cp.current_sequence_order + 1]
        );
 ...
 */
export async function handleBounce(sentEmailId: string, bounceReason: string): Promise<void> {
  try {
    // Get email details
    const sentEmail = await db.queryOne<{
      prospect_id: string | null;
      campaign_id: string | null;
    }>('SELECT prospect_id, campaign_id FROM sent_emails WHERE id = $1', [sentEmailId]);
    ...
    await db.query(
      `UPDATE sent_emails
       ...
       WHERE id = $2`,
      [bounceReason, sentEmailId]
    );

    // Pause campaign prospect
    await pauseCampaignProspect(
      sentEmail.prospect_id,
      sentEmail.campaign_id,
      `Email bounced: ${bounceReason}`
    );

    // Update campaign stats
    await db.query(
      `UPDATE campaigns SET emails_bounced = emails_bounced + 1 WHERE id = $1`,
      [sentEmail.campaign_id]
    );

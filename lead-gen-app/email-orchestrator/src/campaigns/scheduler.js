...
export class CampaignScheduler {
    ...
    async processScheduledSends() {
        try {
            ...
            const pendingSends = await db.query('SELECT * FROM pending_sends LIMIT 100');
            ...
            for (const send of pendingSends.rows) {
                try {
                    ...
                    // Get prospect data
                    const prospect = await db.queryOne('SELECT * FROM prospects WHERE id = $1', [send.prospect_id]);
                    if (!prospect) {
                    ...
                    await this.advanceCampaignProspect(send.campaign_prospect_id, send.current_sequence_order + 1);
                    sent++;
     ...
     */
    async advanceCampaignProspect(campaignProspectId, newSequenceOrder) {
       ...
       WHERE campaign_id = (SELECT campaign_id FROM campaign_prospects WHERE id = $1)
         AND sequence_order = $2`, [campaignProspectId, newSequenceOrder + 1]);
        if (nextSequence) {
         ...
         SET current_sequence_order = $1,
             next_send_at = $2
         WHERE id = $3`, [newSequenceOrder, nextSendAt, campaignProspectId]);
        }
        else {
         ...
         SET current_sequence_order = $1,
             ...
             status = 'completed',
             completed_at = NOW()
         WHERE id = $2`, [newSequenceOrder, campaignProspectId]);
        }
    }
    /**
     * Calculate next send time for a campaign prospect
     */
    calculateNextSendTime(params) {
        const { day_offset, timezone, send_days, send_hours_start, send_hours_end, enrolled_at, } = params;
        ...
        const randomHour = send_hours_start + Math.floor(Math.random() * (send_hours_end - send_hours_start));
     ...
     */
    async isWithinSendingWindow(params) {
     ...
     * Find next send day
     */
    findNextSendDay(currentDay, sendDays, timezone) {
     ...
     */
    async enrollProspectsInCampaign(campaignId, prospectIds) {
        try {
            ...
            for (const prospectId of prospectIds) {
                ...
                const existing = await db.queryOne('SELECT id FROM campaign_prospects WHERE campaign_id = $1 AND prospect_id = $2', [campaignId, prospectId]);
                if (existing) {
                    logger.debug('Prospect already enrolled', { prospect_id: prospectId });
                    continue;
                }
                // Get first sequence
                const firstSequence = await db.queryOne(`SELECT day_offset FROM email_sequences
           WHERE campaign_id = $1 AND sequence_order = 1
           ORDER BY sequence_order ASC LIMIT 1`, [campaignId]);
                if (!firstSequence) {
                ...
                // Enroll prospect
                await db.query(`INSERT INTO campaign_prospects (
            campaign_id, prospect_id, current_sequence_order, next_send_at, status
          ) VALUES ($1, $2, 0, $3, 'active')`, [campaignId, prospectId, nextSendAt]);
                enrolled++;
            }

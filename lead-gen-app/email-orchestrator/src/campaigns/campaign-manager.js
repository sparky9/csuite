...
export class CampaignManager {
    ...
    async createCampaign(params) {
        try {
            const { name, description, from_email, from_name, target_prospect_ids, target_tags, target_status, send_days_of_week, send_hours_start, send_hours_end, send_timezone, tracking_enabled, } = params;
     ...
     */
    async addSequence(params) {
        try {
            await db.query(`INSERT INTO email_sequences (
    ...
    async startCampaign(campaignId) {
        try {
            ...
            const sequenceCount = await db.queryOne('SELECT COUNT(*) as count FROM email_sequences WHERE campaign_id = $1', [campaignId]);
            ...
            const campaign = await db.queryOne('SELECT target_prospect_ids, target_tags, target_status FROM campaigns WHERE id = $1', [
                campaignId,
            ]);
            if (!campaign) {
            ...
            if (campaign.target_prospect_ids && campaign.target_prospect_ids.length > 0) {
                ...
                const prospects = await db.query(`SELECT id FROM prospects ${whereClause}`, values);
            ...
            // Enroll prospects
            const enrolledCount = await campaignScheduler.enrollProspectsInCampaign(campaignId, prospectIds);
    ...
    async getCampaign(campaignId) {
        return await db.queryOne('SELECT * FROM campaigns WHERE id = $1', [campaignId]);
    }
    /**
     * Get campaign sequences
     */
    async getCampaignSequences(campaignId) {
        return await db.query('SELECT * FROM email_sequences WHERE campaign_id = $1 ORDER BY sequence_order ASC', [campaignId]);
    }
    /**

 ...
 */
export function createTrackingPixel(html, trackingPixelId) {
 ...
 */
export function getTrackingPixelUrl(trackingPixelId) {
 ...
 */
export function wrapLinksWithTracking(html, sentEmailId) {
 ...
 */
export function getClickTrackingUrl(sentEmailId, originalUrl) {
    const baseUrl = process.env.TRACKING_BASE_URL || 'https://example.com/track';
 ...
 */
export async function recordTrackingEvent(sentEmailId, eventType, eventData) {
    try {
        await db.query(`INSERT INTO email_tracking (
        ...
        if (eventType === 'open') {
            await db.query(`UPDATE sent_emails SET status = 'delivered', delivered_at = NOW()
         WHERE id = $1 AND status = 'sent'`, [sentEmailId]);
            // Update campaign_prospects open count
            await db.query(`UPDATE campaign_prospects cp
         ...
         WHERE prospect_id = (SELECT prospect_id FROM sent_emails WHERE id = $1)
           ...
           AND NOT EXISTS (
            ...
            await db.query(`UPDATE sent_emails SET status = 'bounced', bounce_reason = $1
         WHERE id = $2`, [eventData?.bounce_reason || 'Unknown', sentEmailId]);
         ...
         WHERE prospect_id = (SELECT prospect_id FROM sent_emails WHERE id = $1)
           AND campaign_id = (SELECT campaign_id FROM sent_emails WHERE id = $1)`, [sentEmailId]);
        }
 ...
 */
export async function getEmailTrackingStats(sentEmailId) {
    try {
        const events = await db.query(`SELECT event_type, occurred_at, clicked_url
       FROM email_tracking
       WHERE sent_email_id = $1
       ORDER BY occurred_at ASC`, [sentEmailId]);
...
export async function getCampaignTrackingStats(campaignId) {
    try {
       ...
       WHERE se.campaign_id = $1 AND se.sent_at IS NOT NULL`, [campaignId]);
        if (!stats) {
            return {
                total_sent: 0,
                total_delivered: 0,

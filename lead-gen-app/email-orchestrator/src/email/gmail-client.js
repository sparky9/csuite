...
export class GmailClient {
    ...
    async initialize() {
        try {
            ...
            if (!clientId || !clientSecret) {
                throw new Error('Gmail OAuth credentials not found. Set GMAIL_CLIENT_ID and GMAIL_CLIENT_SECRET');
            }
    ...
    async loadTokens() {
        try {
            const result = await db.query(`SELECT key, value FROM email_config
    ...
    async saveTokens(tokens) {
        try {
            await db.query(`INSERT INTO email_config (key, value, updated_at)
         VALUES
         ...
         ON CONFLICT (key) DO UPDATE SET
           ...
           updated_at = NOW()`, [tokens.access_token, tokens.refresh_token, tokens.expiry_date.toString()]);

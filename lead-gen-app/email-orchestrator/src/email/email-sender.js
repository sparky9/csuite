...
import { addCompliantFooter, htmlToPlainText, isUnsubscribed, } from '../utils/compliance.js';
...
export class EmailSender {
    ...
    async sendEmail(params) {
        const { to_email, to_name, from_email, from_name, subject_line, body_html, body_plain, campaign_id, sequence_id, prospect_id, tracking_enabled = true, } = params;
        ...
        try {
            ...
            const sentEmailResult = await db.query(`INSERT INTO sent_emails (
    ...
    RETURNING id`, [
        ...
        provider,
            ...
            await db.query(`UPDATE sent_emails SET body_html = $1, body_plain = $2 WHERE id = $3`, [finalHtml, finalPlainText, sentEmailId]);
         ...
         WHERE id = $2`, [result.messageId, sentEmailId]);
           ...
           WHERE campaign_id = $1 AND prospect_id = $2`, [campaign_id, prospect_id]);
            ...
            const sentEmail = await db.queryOne('SELECT * FROM sent_emails WHERE id = $1', [sentEmailId]);
            return sentEmail;
        }
        catch (error) {
           ...
           SET status = 'failed',
               error_message = $1
           WHERE id = $2`, [error.message, error.sentEmailId]);
            }
            throw error;
        }
    }
        ...
        for (const emailParams of emails) {
            try {
                ...
                if (error?.code === 'QUOTA_EXCEEDED') {
                    const provider = error?.provider ?? this.resolveProvider(emailParams.provider);
    ...
    async ensureProviderInitialized(provider) {
        if (provider === 'smtp') {
    ...
    createQuotaError(provider, quota) {
        ...
        error.waitUntil = quota.wait_until;
        return error;
    }
    /**

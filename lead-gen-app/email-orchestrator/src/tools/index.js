...
import { pauseCampaign, resumeCampaign } from '../campaigns/auto-pause.js';
...
export async function handleCreateTemplate(args) {
    try {
        const result = await db.query(`INSERT INTO email_templates (name, category, subject_line, body_template, personalization_instructions, use_ai_enhancement, description)
...
export async function handleGetCampaignStats(args) {
    try {
        const performance = await db.queryOne('SELECT * FROM campaign_performance WHERE id = $1', [args.campaign_id]);
        const sequences = await db.query('SELECT * FROM email_sequences WHERE campaign_id = $1 ORDER BY sequence_order', [args.campaign_id]);
        const recentActivity = await db.query(`SELECT * FROM email_activity_timeline
       WHERE campaign_id = $1
       ORDER BY sent_at DESC LIMIT 20`, [args.campaign_id]);
        const topProspects = await db.query(`SELECT prospect_id, emails_sent, emails_opened, replied
       FROM campaign_prospects
       WHERE campaign_id = $1
       ORDER BY emails_opened DESC LIMIT 10`, [args.campaign_id]);
        return {
            content: [
                {
                    type: 'text',
...
// ============================================================================
export const pauseResumeCampaignTool = {
...
export async function handlePauseResumeCampaign(args) {
    try {
        let result;
...
// ============================================================================
export const getEmailHistoryTool = {
...
export async function handleGetEmailHistory(args) {
    try {
       ...
       LIMIT $2`, [args.prospect_id, args.limit || 50]);
        return {
            content: [
                {
                    type: 'text',
...
// ============================================================================
export const manageUnsubscribesTool = {
...
export async function handleManageUnsubscribes(args) {
    try {
        if (args.action === 'list') {
            const unsubscribes = await db.query('SELECT * FROM unsubscribes ORDER BY unsubscribed_at DESC LIMIT $1', [args.limit || 100]);
            return {
                content: [
                    {
                        type: 'text',
            ...
            await db.query('INSERT INTO unsubscribes (email, unsubscribe_reason) VALUES ($1, $2) ON CONFLICT (email) DO NOTHING', [args.email, args.reason || 'Manual addition']);
            return {
                content: [
                    {
                        type: 'text',
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: JSON.stringify({ success: true, is_unsubscribed: result !== null, details: result }, null, 2),
                    },
                ],
            };
        }
...
// ============================================================================
export const addEmailSequenceTool = {
...
export async function handleAddEmailSequence(args) {
    try {
...
export const startCampaignTool = {
    name: 'start_campaign',
...
export async function handleStartCampaign(args) {
    try {

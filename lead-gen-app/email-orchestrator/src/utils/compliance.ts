...
import type { CompanyInfo, UnsubscribeLink } from '../types/email.types.js';
 ...
 */
export function addUnsubscribeLink(html: string, sentEmailId: string): string {
 ...
 */
export function generateUnsubscribeUrl(sentEmailId: string): string {
...
export async function handleUnsubscribe(
  ...
  try {
    ...
    const sentEmail = await db.queryOne<{ to_email: string; campaign_id: string | null }>(
      'SELECT to_email, campaign_id FROM sent_emails WHERE id = $1',
      [sentEmailId]
    );
    ...
    await db.query(
      `INSERT INTO unsubscribes (email, unsubscribe_reason, campaign_id, sent_email_id)
       VALUES ($1, $2, $3, $4)`,
      [sentEmail.to_email, reason || 'User requested', sentEmail.campaign_id, sentEmailId]
    );
 ...
 */
export async function addCompliantFooter(
  html: string,
  sentEmailId: string
): Promise<string> {
 ...
 */
export function validateEmailCompliance(html: string): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  const hasUnsubscribe =
    html.toLowerCase().includes('unsubscribe') && html.toLowerCase().includes('href=');

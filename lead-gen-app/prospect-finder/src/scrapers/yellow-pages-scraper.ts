...
> {
   ...
   * Extract business listings from current page
   */
  private async extractBusinessListings(page: BrowserInstance['page']): Promise<YellowPagesResult[]> {
    ...
    return await page.evaluate(() => {
      ...
      if (!doc) {
        console.warn('Document not available in page context');
        return results;
      }
  ...
  listings = doc.querySelectorAll(selector);
      ...
      listings.forEach((listing: any, index: number) => {
        try {
          ...
          phoneEls.forEach((el: any, idx: number) => {
            if (idx > 0) {
   ...
   */
  private async hasNextPage(page: BrowserInstance['page']): Promise<boolean> {
    return await page.evaluate(() => {
      const nextSelectors = [
        '.next:not(.disabled)',
        'a.next:not(.disabled)',
      ...
      for (const selector of nextSelectors) {
        const doc = (globalThis as any).document;
        if (!doc) {
          console.warn('Document not available while locating pagination button');
          continue;
        }
   ...
   * Navigate to next page
   */
  private async goToNextPage(page: BrowserInstance['page']): Promise<void> {
    const clicked = await page.evaluate(() => {
      ...
      for (const selector of nextSelectors) {
        ...
        const nextButton: any = doc.querySelector(selector);
        if (nextButton && !nextButton.classList.contains('disabled')) {
          nextButton.click();
          return true;
        }

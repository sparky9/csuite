...
export interface EmailFinderParams {
  ...
  person_names?: Array<{ first_name: string; last_name: string }>; // Optional: for pattern matching
  search_website?: boolean; // Whether to scrape website for emails
  use_hunter_api?: boolean; // Whether to use Hunter.io API
}

export class EmailFinder extends BaseScraper<EmailFinderParams, EmailFinderResult[]> {
  private hunterApiKey: string | null = null;
   ...
   */
  private async scrapeWebsiteForEmails(
    domain: string,
    browser: BrowserInstance
  ): Promise<string[]> {
   ...
   */
  private async verifyEmailWithHunter(
    email: string

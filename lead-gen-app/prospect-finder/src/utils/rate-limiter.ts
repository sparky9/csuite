...
export class RateLimiter {
  ...
  constructor(configPath: string = './config/scraper-limits.json') {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.config = JSON.parse(configData) as RateLimitConfig;
   ...
   */
  async checkLimit(source: keyof RateLimitConfig): Promise<{
    allowed: boolean;
    reason?: string;
    retryAfterMs?: number;
  }> {
    ...
    const requestsLastMinute = bucket.requestTimes.filter(
   ...
   */
  private getBucket(source: keyof RateLimitConfig): TokenBucket {
  ...
  getAllStats() {
    ...
    const sources = Object.keys(this.config).filter(
      (k) => k !== 'global'
    ) as Array<keyof RateLimitConfig>;

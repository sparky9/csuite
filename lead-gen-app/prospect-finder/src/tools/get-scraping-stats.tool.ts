...
export async function getScrapingStatsTool(args: unknown, dbConnected: boolean, userId?: string) {
  // Validate input
...
async function getCompanyStats(timeRange: string, userId?: string) {
  const timeFilter = getTimeFilter(timeRange);

  const whereConditions = [];
  if (userId) whereConditions.push(`user_id = '${userId}'`);
  if (timeFilter) whereConditions.push(`created_at >= NOW() - INTERVAL '${timeFilter}'`);
...
async function getJobStats(timeRange: string, userId?: string) {
  const timeFilter = getTimeFilter(timeRange);
...
async function getQualityStats(userId?: string) {
...
function formatSuccessResponse(stats: any, params: z.infer<typeof GetScrapingStatsSchema>, fromDatabase: boolean) {
  ...
  if (fromDatabase && stats.quality.length > 0) {
    ...
    stats.quality.forEach((tier: any) => {

...
export async function findDecisionMakersTool(args, dbConnected, userId) {
    // Validate input
    const params = FindDecisionMakersSchema.parse(args);
    logger.info('Finding decision makers', params);
    // Get company info
    let company = null;
    let existingDecisionMakers = [];
    if (dbConnected) {
        try {
...
async function getDecisionMakersFromDatabase(companyId, userId) {
    const query = userId
        ? `SELECT
        id,
        full_name,
...
async function storeDecisionMakerInDatabase(companyId, dm, userId) {
    ...
    if (existing) {
        ...
        await db.query(`UPDATE decision_makers SET
        full_name = $1,
        first_name = $2,
        last_name = $3,
        title = $4,
        ...
        confidence_score = $6,
        updated_at = NOW()
      WHERE id = $7`, [dm.full_name, dm.first_name, dm.last_name, dm.title, dm.email, dm.confidence_score, existing.id]);
...
function generateMockDecisionMakers(params) {
...
function formatSuccessResponse(company, decisionMakers, params, dataSource) {
    const sourceText = dataSource === 'linkedin'
...
//# sourceMappingURL=find-decision-makers.tool.js.map
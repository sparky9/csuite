...
export async function enrichCompanyTool(args: unknown, dbConnected: boolean, userId?: string) {
  // Validate input
    ...
    if (params.sources.includes('linkedin') && company.name) {
      try {
        ...
        if (result.success && result.data) {
          ...
          if (
            params.fields.includes('employee_count') &&
...
async function updateCompanyInDatabase(company: any, newQualityScore: number) {
...
function generateMockEnrichment(params: z.infer<typeof EnrichCompanySchema>) {
  const enrichedData: any = {
...
function formatSuccessResponse(
  company: any,
  enrichedData: any,
  _params: z.infer<typeof EnrichCompanySchema>,
  dataSource: 'database' | 'scraped' | 'mock'
) {
  const sourceText =
    dataSource === 'scraped'
  ...
  if (enrichedData.fields_enriched.length === 0) {
    ...
    Object.entries(enrichedData.changes).forEach(([field, change]: [string, any]) => {
      resultsText += `${field}:\n`;

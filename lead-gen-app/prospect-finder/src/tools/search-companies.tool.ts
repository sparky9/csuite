...
import { YellowPagesResult, GoogleMapsResult } from '../types/prospect.types.js';
...
export async function searchCompaniesTool(args: unknown, dbConnected: boolean, userId?: string) {
  // Validate input
  ...
  try {
    ...
    let scrapedCompanies: any[] = [];
    let dataSource: 'yellow_pages' | 'google_maps' = 'yellow_pages';
    ...
    // All scraping and database methods failed - return clear error
    return formatErrorResponse('Unable to find companies. All data sources are unavailable or failing.', params);
  }
}
 ...
 */
function convertYellowPagesToStandard(company: YellowPagesResult): any {
 ...
 */
function convertGoogleMapsToStandard(company: GoogleMapsResult): any {
 ...
 */
function calculateYellowPagesQualityScore(company: YellowPagesResult): number {
  let score = 0;

  // Phone (30% - critical for B2B)
  if (company.phone) score += 0.30;

  // Website (25%)
  if (company.website) score += 0.25;

  // Complete address (20%)
  if (company.address && company.city && company.state) score += 0.20;

  // Category (10%)
  if (company.category) score += 0.10;

  // Years in business (10% - unique to Yellow Pages!)
  if (company.years_in_business) score += 0.10;

  // BBB rating (5%)
  if (company.bbb_rating) score += 0.05;
 ...
 */
function calculateGoogleMapsQualityScore(company: GoogleMapsResult): number {
  let score = 0;
  let maxScore = 0;

  // Phone (25%)
  maxScore += 0.25;
  if (company.phone) score += 0.25;
  ...
  // Rating (15%)
  maxScore += 0.15;
  if (company.rating && company.rating >= 4.0) score += 0.15;
  else if (company.rating && company.rating >= 3.0) score += 0.075;

  // Review count (15%)
  maxScore += 0.15;
  if (company.review_count && company.review_count >= 50) score += 0.15;
  else if (company.review_count && company.review_count >= 10) score += 0.075;
 ...
 */
async function storeCompaniesInDatabase(
  companies: any[],
  _params: z.infer<typeof SearchCompaniesSchema>,
  dataSource: 'yellow_pages' | 'google_maps',
  userId?: string
): Promise<void> {
...
function calculateCompleteness(company: GoogleMapsResult): number {
  const fields = [
    'name',
    'phone',
    'website',
  ...
  const filledFields = fields.filter((field) => company[field as keyof GoogleMapsResult]);
  return (filledFields.length / fields.length) * 100;
}

async function searchCompaniesFromDatabase(params: z.infer<typeof SearchCompaniesSchema>) {
  const query = `
    SELECT
      id,
      name,
...
function formatSuccessResponse(
  ...
  dataSource: 'yellow_pages' | 'google_maps' | 'database' | 'mock'
) {
...
function formatErrorResponse(error: string, params: z.infer<typeof SearchCompaniesSchema>) {

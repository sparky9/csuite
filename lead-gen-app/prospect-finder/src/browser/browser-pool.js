...
export class BrowserPool {
    ...
    headless;
    reuseCount;
    constructor(proxyManager, maxConcurrent = 2, headless = true, reuseCount = 50) {
     ...
     */
    async release(instanceId, success = true) {
     ...
     */
    async waitForAvailableBrowser(timeoutMs = 30000) {
    ...
    async closeAll() {
        ...
        const closePromises = Array.from(this.instances.keys()).map((id) => this.closeBrowserInstance(id));
...
export function getBrowserPool(proxyManager) {
    if (!browserPool) {

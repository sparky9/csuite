...
class DatabaseClient {
  ...
  async connect(): Promise<void> {
    ...
    if (!databaseUrl) {
      ...
      logger.info('Time & Billing database using in-memory storage');
      return;
    }
    ...
    try {
      ...
      logger.info('Time & Billing database connected');
    } finally {
      client.release();
    }
  }
  ...
  async disconnect(): Promise<void> {
    ...
    await this.pool.end();
    this.pool = null;
    logger.info('Time & Billing database disconnected');
  }

  async applySchemaFromFile(): Promise<void> {
    await this.applySchema();
  }

...
async function main() {
    ...
    const savedTemplate = unwrap(await handleTemplateSave({
        name: templateName,
    ...
    console.log({ count: templateList.count, latestNames: templateList.templates?.slice(0, 3)?.map((t) => t.name) });
    ...
    const proposal = unwrap(await handleProposalGenerate({
        templateId,
        client: {
    ...
    const sentProposal = unwrap(await handleProposalSend({ proposalId, note: 'Sent via smoke test harness.' }));
    ...
    const contract = unwrap(await handleContractGenerate({
        proposalId,
    ...
    const sentContract = unwrap(await handleContractSend({
        contractId,
    ...
    console.log({
        contacts: contractStatus.contract?.contacts?.map((c) => ({ role: c.role, reminderCount: c.reminderCount })),
        sentAt: contractStatus.contract?.sentAt,
    });
    divider('Signature reminder');
    const reminder = unwrap(await handleSignatureRemind({
        contractId,
        tone: 'professional',
        reminderType: 'first',
    }));
...
//# sourceMappingURL=test-proposal-tools.js.map
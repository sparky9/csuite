...
function sanitizeCurrency(code?: string): string | undefined {
...
export function normalizeExchangeRate(currency: string, exchangeRate?: number): number {
...
export function toBaseCurrency(amount: number, currency?: string, exchangeRate?: number): number {
  const normalizedCurrency = normalizeCurrency(currency);
  const rate = normalizeExchangeRate(normalizedCurrency, exchangeRate);
  return Number((amount * rate).toFixed(2));
}

export function getFallbackExchangeRate(currency: string): number | undefined {
  const normalized = normalizeCurrency(currency);
  return normalized === BASE_CURRENCY ? 1 : FALLBACK_RATES[normalized];
}

export function describeExchangeRate(currency?: string, exchangeRate?: number): {
  currency: string;
  exchangeRate: number;
  baseCurrency: string;
} {

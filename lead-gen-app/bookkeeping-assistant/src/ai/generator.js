...
import { EXPENSE_CATEGORIES, INCOME_CATEGORIES, TAX_BRACKETS, } from '../types/bookkeeping.types.js';
import { BASE_CURRENCY, describeExchangeRate, normalizeCurrency, normalizeExchangeRate, toBaseCurrency, } from '../utils/currency.js';
...
function normalizeCategory(category, type) {
    ...
    if (type === 'income') {
        return INCOME_CATEGORIES[normalized]
            ? INCOME_CATEGORIES[normalized]
            : category;
    }
    if (type === 'expense' || type === 'transfer') {
        return EXPENSE_CATEGORIES[normalized]
...
export function createExpenseRecord(params) {
...
function buildInvoiceHtml(invoice, params) {
...
function buildInvoiceText(invoice, params) {
...
function generateReportRecommendations(type, snapshot) {
...
function buildReportDetails(includeDetails, snapshot) {
    if (!includeDetails) {
        return undefined;
    }
...
export function buildFinancialReport(params, ledger) {
...
function calculateBracketedTax(income, year) {
    const brackets = TAX_BRACKETS[year] ?? TAX_BRACKETS[2023];
...
export function estimateTax(params, ledger) {
    const snapshot = summarizeLedger(ledger);
    const deductions = {};
...
export function categorizeTransactions(rawTransactions) {
...
export function planBudget(period, totalBudget, categories, historicalData, goals) {
...
function nextPeriodLabel(periodType, index, historicalLast) {
...
export function forecastCashFlow(periodType, forecastPeriod, historicalData, assumptions) {
...
export function ledgerFromTransactions(rows) {
    return rows.map(row => {
...
export function synthesizeLedger(seed, months = 6) {
    const entries = [];
    ...
    for (let i = 0; i < months; i++) {
        ...
        const incomeIndex = Math.max(0, Math.min(incomeCategories.length - 1, Math.floor(seededNumber(`${seed}:income:${i}`, 0, incomeCategories.length - 1.0001))));
        const expenseIndex = Math.max(0, Math.min(expenseCategories.length - 1, Math.floor(seededNumber(`${seed}:expense:${i}`, 0, expenseCategories.length - 1.0001))));

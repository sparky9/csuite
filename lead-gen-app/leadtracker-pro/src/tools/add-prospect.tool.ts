...
import type { AddProspectInput, Prospect } from '../types/leadtracker.types.js';
...
export async function addProspectTool(args: any, _dbConnected?: boolean, userId?: string) {
  try {
    // Validate input
    const input = AddProspectSchema.parse(args) as AddProspectInput;
    ...
    const prospect = await db.queryOne<Prospect>(insertQuery, insertParams);

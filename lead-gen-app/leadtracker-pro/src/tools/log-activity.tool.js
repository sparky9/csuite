...
export async function logActivityTool(args, dbConnected, userId) {
    try {
        // Validate input
        ...
        const activity = await db.queryOne(activityQuery, activityParams);
        if (!activity) {
        ...
        if (input.follow_up_date) {
         ...
         WHERE id = $2
           AND (next_follow_up IS NULL OR next_follow_up > $1)`, [new Date(input.follow_up_date), input.prospect_id]);
        }
        ...
        const callOutcomeText = input.call_outcome
            ...
            : '';
        const durationText = input.call_duration_seconds && input.call_duration_seconds > 0
...
//# sourceMappingURL=log-activity.tool.js.map
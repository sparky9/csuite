...
export async function updateProspectStatusTool(args, dbConnected, userId) {
    try {
        // Validate input
        const input = UpdateProspectStatusSchema.parse(args);
        ...
        const currentProspect = await db.queryOne(selectQuery, selectParams);
        if (!currentProspect) {
            throw new Error(`Prospect not found: ${input.prospect_id}`);
        }
        const oldStatus = currentProspect.status;
        // Update status
        const updatedProspect = await db.queryOne('UPDATE prospects SET status = $1, updated_at = NOW() WHERE id = $2 RETURNING *', [input.new_status, input.prospect_id]);
        if (!updatedProspect) {
      ...
      ) VALUES ($1, $2, $3, $4)`, [input.prospect_id, 'note', 'Status Change', activityNote]);
...
//# sourceMappingURL=update-prospect-status.tool.js.map
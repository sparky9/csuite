...
import type { SearchProspectsInput } from '../types/leadtracker.types.js';
...
export async function searchProspectsTool(args: any, _dbConnected?: boolean, userId?: string) {
  try {
    // Validate input
    const input = SearchProspectsSchema.parse(args) as SearchProspectsInput;
    ...
    const prospects: any[] = result.rows;
    ...
    if (totalCount === 0) {
      ...
      prospects.forEach((p: any, index: number) => {
        ...
        if (p.phone) responseText += ` | Phone: ${p.phone}`;
        if (p.email) responseText += ` | Email: ${p.email}`;
        responseText += '\n';
        if (p.city && p.state) responseText += `   Location: ${p.city}, ${p.state}\n`;
        if (p.tags && p.tags.length > 0) responseText += `   Tags: ${p.tags.join(', ')}\n`;
        if (p.deal_value) responseText += `   Deal Value: $${p.deal_value.toLocaleString()}\n`;
        responseText += `   Contacts: ${p.contact_count} | Activities: ${p.activity_count}`;
        if (p.pending_follow_ups > 0) responseText += ` | ⏰ Follow-ups: ${p.pending_follow_ups}`;
        responseText += '\n';

...
export async function addContactTool(args, dbConnected, userId) {
    try {
        // Validate input
        ...
        if (input.is_primary) {
            await db.query('UPDATE contacts SET is_primary = FALSE WHERE prospect_id = $1', [input.prospect_id]);
        }
      ...
      RETURNING *`;
        ...
        const contact = await db.queryOne(insertQuery, insertParams);
        if (!contact) {
...
//# sourceMappingURL=add-contact.tool.js.map
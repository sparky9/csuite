...
import type { GetFollowUpsInput } from '../types/leadtracker.types.js';
...
export async function getFollowUpsTool(args: any, _dbConnected?: boolean, userId?: string) {
  try {
    // Validate input
    const input = GetFollowUpsSchema.parse(args || {}) as GetFollowUpsInput;
    ...
    if (followUps.length === 0) {
      ...
      followUps.forEach((f, index) => {
        ...
        if (isOverdue) responseText += ' 🔴 OVERDUE';
        responseText += '\n';
        ...
        if (f.reminder_type) responseText += `   Type: ${f.reminder_type}\n`;
        if (f.reminder_note) responseText += `   Note: ${f.reminder_note}\n`;

        if (f.contact_name) {
          responseText += `   Contact: ${f.contact_name}`;
          if (f.contact_phone) responseText += ` | ${f.contact_phone}`;
          if (f.contact_email) responseText += ` | ${f.contact_email}`;
          responseText += '\n';
        }

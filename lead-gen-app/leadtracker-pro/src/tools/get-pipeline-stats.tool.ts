...
import type { GetPipelineStatsInput } from '../types/leadtracker.types.js';
...
export async function getPipelineStatsTool(args: any, _dbConnected?: boolean, userId?: string) {
  try {
    // Validate input
    const input = GetPipelineStatsSchema.parse(args || {}) as GetPipelineStatsInput;
    ...
    const totalsResult = await db.queryOne<any>(totalsQuery, userParams);
    ...
    const groupResult = await db.query<any>(groupQuery, userParams);
